...
public interface ProductRepository extends JpaRepository<Product, Long> /* format- < Model, Data_Type of Model's Primary Key>   */ {
    ...
    @Query(value= """
        ...
        LIMIT :limit
        """, nativeQuery= true)
    List<Product> findRandomAvailableProductsWithQty(ProductStatus productStatus, int limit);
...
//
//    Page<Product> findAllByCategory_Id(Long categoryId, Pageable pageable);// maybe used by customer
//
//    Optional<Product> findByUserAndTitle(User user, String title);
//
//    Optional<Product> findByUserAndId(User user , Long id);

    Optional<Product> findByTitleAndModel(String title,String model);
...
//    List<Product> getProductByCategoryIdNativeQuery(@Param("categoryId") Long categoryId);
    ...
    @Query("select p.title as name, p.id as id from Product p where p.category.id=: categoryId")
